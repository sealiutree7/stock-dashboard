...
let lastPrices={};
let usStore={},twStore={},usThemeStore={},twThemeStore={};
const AUTO_MS=5000;
...
async function loadUS(){const syms=list("usSymbols");chips("usChips",syms);usStore={};const data=await api(`/api/session-quotes?symbols=${encodeURIComponent(syms.join(","))}`);$("session").textContent=data.session?.label||"Session";$("usCards").innerHTML=(data.results||[]).map(x=>x.ok?usCard(x.quote.symbol,x.quote):`<div class="card"><div class="symbol"><span>${x.symbol}</span><span>Error</span></div><div class="card-meta">${x.error}</div></div>`).join("");document.querySelectorAll("#usCards .card[data-symbol]").forEach(c=>c.onclick=()=>selectSymbol(c.dataset.symbol));}
async function loadTW(){const codes=list("twSymbols");chips("twChips",codes);twStore={};const data=await api(`/api/tw-yahoo-quotes?codes=${encodeURIComponent(codes.join(","))}`);$("twCards").innerHTML=Object.values(data||{}).map(q=>q.ok?twCard(q):`<div class="card"><div class="symbol"><span>${q.code}</span><span>Error</span></div><div class="card-meta">${q.error||"no data"}</div></div>`).join("");}
async function loadThemeUniverses(){const uniq=a=>[...new Set(a)];const usCodes=uniq(US_THEMES.flatMap(t=>t.symbols));const twCodes=uniq(TW_THEMES.flatMap(t=>t.symbols));const usData=await api(`/api/session-quotes?symbols=${encodeURIComponent(usCodes.join(","))}`);const twData=await api(`/api/tw-yahoo-quotes?codes=${encodeURIComponent(twCodes.join(","))}`);usThemeStore={};(usData.results||[]).forEach(x=>{if(x.ok)usThemeStore[x.quote.symbol]=x.quote});twThemeStore={};Object.values(twData||{}).forEach(q=>{if(q.ok)twThemeStore[q.code]=q});heat("us",US_THEMES,usThemeStore);heat("tw",TW_THEMES,twThemeStore);}
...
async function refreshAll(withThemes=true){localStorage.setItem("apiBase",$("apiBase").value.trim());localStorage.setItem("usSymbols",$("usSymbols").value);localStorage.setItem("twSymbols",$("twSymbols").value);$("refreshStatus").textContent="Updating...";try{await loadUS();await loadTW();if(withThemes)await loadThemeUniverses();$("refreshStatus").textContent=`Auto updated ${new Date().toLocaleTimeString()} · 5s`;}catch(e){$("refreshStatus").textContent=`Error: ${e.message}`;}}
