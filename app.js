...
function cardQuote(label,q){
  ...
  return `<div class="card">
    ...
    <div class="data-source-note">${q.source||""}${q.sourceSymbol?`｜${q.sourceSymbol}`:""}</div>
  </div>`;
}
...
async function loadTWIndex(){
  const symbols=TW_INDEX.map(x=>x.symbol);
...
function renderThemeFocusCards(prefix,theme){
  ...
  el.innerHTML=theme.symbols.map(sym=>{
    ...
    if(prefix==="tw"){
      ...
      if(!q){
        return `<div class="card">
          <div class="symbol"><span>${name}</span><span>${sym}</span></div>
          <div class="card-meta">目前沒有報價資料。可能是資料源暫時沒有回應，但此卡片會保留。</div>
        </div>`;
      }
      return `<div class="card" data-code="${q.code}" data-market="${q.market}">
    ...
    if(!q){
      return `<div class="card"><div class="symbol"><span>${sym}</span><span>No data</span></div><div class="card-meta">目前沒有報價資料。</div></div>`;
    }
    const m=q.main||{};
    return `<div class="card" data-symbol="${q.symbol}">
      <div class="symbol"><span>${q.symbol}</span><span>${q.symbol}</span></div>
